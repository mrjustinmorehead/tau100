...
exports.handler = async (event) => {
  try {
    ...
    if (idx === -1) {
      return { statusCode: 200, headers: { 'content-type': 'text/html' }, body: '<h1>Already Confirmed</h1><p>This code is not in pending; it may already be confirmed.</p><p><a href=\"/\">Return</a></p>' };
    }
    ...
    if (dup !== -1) { regList.splice(dup, 1, rec); } else { regList.push(rec); }

    await pending.setJSON(pendList);
    await registrants.setJSON(regList);
    return { statusCode: 302, headers: { Location: '/#register?confirmed=1' } };
  } catch (e) {
