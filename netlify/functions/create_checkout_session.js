...
exports.handler = async (event) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: { currency: 'usd', unit_amount: 10000, product_data: { name: 'Tau Centennial Registration' } },
        quantity: 1,
      }],
      customer_email: email,
      success_url: `${process.env.SITE_URL}/?success=1`,
      cancel_url: `${process.env.SITE_URL}/?canceled=1`,
      metadata: { name, email, phone, yearJoined: String(y), photoUrl: photoUrl || '', optInPublic: optInPublic ? '1' : '0' },
    });
