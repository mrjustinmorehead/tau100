...
export const handler = async (event) => {
  ...
  const regs = stores.registrants(); const keys = await regs.list();
  const rows = [
  ...
  for (const k of keys.blobs) {
    const r = await regs.getJSON(k.key); if (!r) continue;
    const line = [
      (r.name||"").replace(/,/g," "),(r.email||"").replace(/,/g," "),(r.phone||"").replace(/,/g," "),
      r.yearJoined||"", r.tshirtSize||"", (r.packageName||"").replace(/,/g," "),
      r.packageAmount||"", r.verification||"", r.confirmedAt||"", r.createdAt||""
    ].join(",");
    rows.push(line);
  }
