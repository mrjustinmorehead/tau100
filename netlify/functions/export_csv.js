...
exports.handler = async (event) => {
  try {
    ...
    let rows = await db.getJSON(); if (!Array.isArray(rows)) rows = common.normalizeToArray(rows);
    const cols = ['name','email','phone','yearJoined','tshirtSize','optInPublic','packageName','packageAmount','ts'];
